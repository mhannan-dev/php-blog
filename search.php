...
<div class="flex flex-col gap-6">
    <div class="glass-card rounded-2xl p-6 shadow-md shadow-black/10">
        <h2 class="text-xl font-bold font-outfit text-white">
            Search Results for: <span class="gradient-text">"<?php echo Format::e($searchTerm); ?>"</span>
        </h2>
    </div>

    <?php if ($searchResult && $searchResult->num_rows > 0): ?>
        <div class="flex flex-col gap-6">
            <?php while ($post = $searchResult->fetch_assoc()): ?>
                <!-- Search Result Card -->
    ...
    <?php else: ?>
        <div class="glass-card rounded-3xl p-12 text-center text-slate-400 shadow-md">
            ...
            <p>No results found for <strong>"<?php echo Format::e($searchTerm); ?>"</strong>.</p>
        </div>
    <?php endif; ?>
</div>
