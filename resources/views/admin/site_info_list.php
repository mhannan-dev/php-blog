...
<div class="glass-card rounded-3xl p-6 sm:p-8 shadow-xl shadow-black/5 flex flex-col gap-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
        <div class="flex flex-col gap-1">
            <h2 class="text-xl font-bold font-outfit text-white">Site Informations</h2>
            <p class="text-slate-400 text-xs font-medium">Manage primary metadata rows in the database</p>
        </div>
        <a href="titleslogan.php" 
           class="px-4 py-2 bg-brand-500 hover:bg-brand-600 active:bg-brand-700 text-white text-xs font-bold rounded-xl transition-all duration-200 shadow-md shadow-brand-500/10 cursor-pointer flex items-center gap-1.5 shrink-0">
            <i class="fa-solid fa-plus-circle text-[10px]"></i> Create New Settings
        </a>
    </div>
    ...
    <div class="overflow-x-auto rounded-2xl border border-white/5 bg-slate-900/30">        
        <table class="w-full text-left text-sm border-collapse">
            ...
            <tbody class="divide-y divide-white/5 text-slate-350">
                <?php
                $infos = $siteModel->getAllSiteInfo();
                if ($infos && $infos->num_rows > 0) {
                    $i = 0;
                    while ($result = $infos->fetch_assoc()) {
                        $i++;
                        ?>
                        <tr class="hover:bg-white/2 transition-colors duration-150">
                            <td class="px-6 py-4 text-slate-400 font-medium"><?php echo $i; ?></td>
                            <td class="px-6 py-4 text-slate-200 font-bold max-w-[150px] truncate"><?php echo Format::e($result['title']); ?></td>
                            <td class="px-6 py-4 text-slate-300 max-w-[200px] truncate"><?php echo Format::e($result['slogan']); ?></td>
                            <td class="px-6 py-4">
                                <?php if ($result['logo']): ?>
                                    <img src="<?php echo Format::e($result['logo']); ?>" 
                                         class="h-10 w-24 rounded-lg object-contain border border-white/10 bg-slate-950 shrink-0" alt="Site Logo">
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4 text-right text-xs font-bold flex items-center justify-end gap-2.5">
                                <a href="edit_site_info.php?info_id=<?php echo (int) $result['id']; ?>"
                                ...
                                <a onclick="return confirm('Are you sure want to delete this item?')" 
                                   href="?site_info_id=<?php echo (int) $result['id']; ?>"
                    ...
                    ?>
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-slate-500 italic">No settings rows found.</td>
                    </tr>
                    <?php
                }
                ?>
